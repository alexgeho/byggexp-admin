...
- **Onboarding wizard: form-steps now complete only on real action in the wizard** (`OnboardingWizard.jsx`). Bug: the "Add your team" step showed a green ✓ on a fresh company because a seed/demo worker made `teamCount > 1` (state-based `done` in `activation.buildOnboardingSteps`). Fix: persist a per-company `wizardDone` set (localStorage `byggexp-onboarding-wizard-done:{companyId}`), marked in `handleCreated` from the open step's `activeKey`. `base` now overrides `done` for **form-based** steps (in `FORM_REGISTRY`: project/team/task/tools/client/article/company) with `wizardDone[key]`; the navigated `billing` step keeps real-state `done`. So seed/demo data never auto-completes a step. NOTE: dashboard `OnboardingChecklist` still uses state-based counts (may show a step done that the wizard shows as not-done) — left as-is for now.
