...
### ПРОБНЫЙ ПЕРИОД
- Владелец: 2 недели со всеми функциями. Бэкенд `TRIAL_DAYS` 30 → 14 (checkout = self-signup), админка «First month free» → «2 weeks free with all features» (10 словарей).
...
### СДЕЛАНО
**Окна / формы — ПРАВИЛО (в памяти `feedback_primary_action_top`): синяя кнопка (Сохранить/Создать/Выбрать) всегда НАВЕРХУ и не уезжает при прокрутке.**
- `AdminModal` рендерит кнопки в шапке справа от заголовка (шапка не скроллится) → все ~30 окон разом. Cancel/Save по умолчанию через `t()` (было «Cancel» в RU). (`1cee56c`)
- «Создать проект» — кнопки не было вообще (`footer={null}` остался от визарда после `a0e0956`) → вернул. (`52c5ab0`)
- 5 окон на голом antd `Modal` → `AdminModal`: AssignmentEdit, BulkScan, BulkScanInvoice, BulkPlanning, UserBulkImport. (`3ebd1ba`)
- Вкладка «Настройки» проекта: «Сохранить изменения» наверху формы, sticky под шапкой 82px. (`3ebd1ba`)
- «Адрес проекта» (ProjectLocationPicker): выбранное место + зона активации НАД картой. (`1cee56c`)
- Название проекта: клик в поле → зеркальный адрес стирается; ушёл пустым → адрес обратно. (`82ad2f9`)

**Сетка часов (Смены → Часы), `HoursPage.jsx` — ПРАВИЛО в памяти `feedback_hours_planned_visible`:**
- Обед вычитался ДВАЖДЫ: бэкенд (`hours.service.ts schedulePlanned`) уже отдаёт planned НЕТТО по `shiftSchedule.lunchMinutes` проекта (07–16 = 8), FE вычитал ещё раз → 7. Теперь правило «Неоплачиваемый обед» (шестерёнка) только для GPS/Вручную; planned показывается/редактируется/Fill как есть. (`1e919a0`; промежуточный `e17a64c` с grossFromNet откатан им же)
- Ввод 8 в пустую ячейку не сохранялся (== графику → «без изменений»). Теперь любая сохранённая коррекция = ввод (`reallyEdited = cell.edited`); «Копировать → след. период» копирует только реальные правки. (`1eea3a4`)
- График виден: в «Запланировано» сегодня+будущие дни показывают план проекта и входят в итог; прошлые без отметок = прочерк (неявка). Ячейки штампуются `date` в `workers` useMemo; `upcomingPlan()` в `isBlank`. (`e4c49a7`)
- Поле выбора проекта шире (340px, `.hours-project-select`).

**Не код:**
- **Apple Developer: миграция Individual → Organization RealMar AB** (case 102968042833). Apple подтвердил eligibility; пользователь 23.09 ответил «proceed» (2FA проверена ВКЛ; byggexp.se/om-oss показывает RealMar AB). Память `project_apple_org_migration`.
- Письмо Rimantas (Geal): напоминание про просроченные фактуры 47/48 + ByggExp на литовском + приложение в сторах + YouTube-канал (@ByggExp).

### ▶ СЛЕДУЮЩИЕ ШАГИ (продолжить отсюда)
1. **Apple-миграция:** ждём письмо Apple (ссылка на подтверждение данных / звонок на +46 70-757 75 75 / возможно новое соглашение/оплата). Пока идёт — НЕ делать новые iOS EAS-сборки (CIP закрыт); `eas update` можно. Когда юзер пришлёт письмо — вести по шагам.
2. **Проверить с юзером вживую:** кнопки в шапке окон (любое создание/редактирование); сетка часов — ввод 8 в пустую ячейку, 07–16 = 8, будущие дни видны.
3. **Возможные хвосты по часам:** ячейки, правленные сегодня ~12:30–13:20 (до `1e919a0`), могли сохраниться +1 ч (grossFromNet) → перебить вручную. Старые строки от прошлых «Копировать → след. период» теперь считаются вводом (закрывают прочерк неявки) → «Сбросить» в шестерёнке.
4. **Открыто:** «все окна закрываются кликом снаружи» — AdminModal уже `maskClosable`; спросить у юзера, КАКОЕ окно не закрылось (на скрине была страница проекта, не модалка).
5. **Mobile OTA** (с 21.09, всё ещё ждёт юзера): `cd tot-bygghub-mobile-app-ios && eas update --branch production` (runtimeVersion 1.1.0).
6. Хвост прошлых сессий: React Compiler глобально; Bankimport bulk-категоризация/split; UX-проход Schedule; dup-хелперы.
