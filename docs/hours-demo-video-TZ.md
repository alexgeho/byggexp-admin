...
- **Подсветка «факт vs план»** (сравнивается GPS `actual` vs `planned`, порог = grace, по умолч. 20 мин):
  - Отработал **больше** плана → **зелёный** фон (`flag-over`, токен `--h-over-*`).
  - Отработал **меньше** плана → **янтарный** фон (`flag-under`, токен `--h-under-*`).
  - Ровно по плану → нейтральная **лаванда** (`planned-fill`, токен `--h-plan-bg`).
  - Работает во всех видах (Planned/GPS/Manual).

При текущих демо-данных: обычные дни у большинства = зелёные (GPS чуть выше плана),
у Raderad (+3 %) ≈ нейтральные, короткие дни = янтарные.
