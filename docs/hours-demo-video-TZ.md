...
> **СТАТУС 2026-08-30 — демо-обвязка внедрена в прод и удалена.**
> Отсутствие (absent, оранжевый прочерк «–») больше НЕ фейк: теперь оно приходит из
> **одобренной заявки Frånvaro** (`src/leave`, `status = approved`). В `getGrid`
> такие дни у работника помечаются `absent` — оранжевый «–», часы не считаются,
> план вычитается из Total Planned. Временный эндпоинт `POST /hours/demo`
> (`demoAdjust` + `DemoAdjustDto`) и поле `Shift.demoAbsent` **удалены**. GPS/Manual
> в бою — реальные (клок-ины + ручной ввод работника). Консольные сниппеты ниже
> (разделы 3–4) больше не работают и оставлены только как история.
