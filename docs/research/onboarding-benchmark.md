...
| G5 | **Server-persisted** прогресс онбординга (focus+view на Company; localStorage = кэш) | ✅ | P2 |
| G6 | **Training-video** URLs — 7 реальных app-видео для worker-аудитории (admin dashboard ещё не снят) | ✅ | P3 |
...
- **2026-09-04 (5)** — **онбординг доведён до 100% best-practice.** **G5 закрыт**: server-persist прогресса — `OnboardingState {focus,view}` на Company (BE: schema+`setOnboarding`+`PATCH /company/:id/onboarding`; FE: reconcile в `OnboardingChecklist` — сервер выигрывает при наличии данных, иначе миграция localStorage вверх; localStorage = кэш/offline-fallback). **G6 закрыт**: 7 реальных app-видео в Help для worker-аудитории. **+ completion-celebration** («You're all set! 🎉») вместо тихого исчезновения (best-practice финал; триггерится только при завершении в сессии). Остаётся только **G3 demo-seed** — намеренно маргинален (empty states G4 закрывают пользу) + риск [[project_company_scoping_debt]].
