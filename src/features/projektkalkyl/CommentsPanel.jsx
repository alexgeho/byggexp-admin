    ...
    <div style={{ marginTop: 20, border: '1px solid var(--border,#e2e8f0)', borderRadius: 12, padding: '16px 18px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Input.TextArea placeholder={t('Write a comment…')} value={text} onChange={(e) => setText(e.target.value)} autoSize={{ minRows: 1, maxRows: 4 }} />
          <Button type="primary" loading={busy} onClick={submit}>{t('Send')}</Button>
        </div>
      </div>
    </div>
  );
