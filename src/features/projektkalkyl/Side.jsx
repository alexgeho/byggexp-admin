...
export default function Side({ money, t, title, tables, totals, totalColor, patchTable, moveTable, removeTable, onAdd, onImport, onScan, onScanFiles, onToggleDetail, detailCount = 0, onShowDetail, scanEnabled }) {
  return (
    <div style={{ flex: '1 1 460px', minWidth: 320, display: 'flex', flexDirection: 'column' }}>
      ...
      <Button icon={<PlusOutlined />} onClick={onAdd} style={{ marginBottom: detailCount > 0 ? 6 : 16, alignSelf: 'flex-start' }}>{t('Add table')}</Button>
      {detailCount > 0 ? (
        <button type="button" onClick={onShowDetail}
          style={{ marginBottom: 16, alignSelf: 'flex-start', background: 'none', border: 'none', padding: 0, cursor: onShowDetail ? 'pointer' : 'default', color: 'var(--muted,#64748b)', fontSize: 12, textDecoration: onShowDetail ? 'underline' : 'none' }}>
          + {detailCount} {t('detailed sheets (in the total)')}
        </button>
      ) : null}
